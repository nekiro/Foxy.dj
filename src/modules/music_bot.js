...
const queue = async (guild) => {
  ...
  let index = 1;
  let msg = await queue.channel.send(
    `Playlist: ${
...
const addToQueue = async (message, skipSelection) => {
  let queue = queues.get(message.guild.id);
  if (!queue) {
    join(message);
    queue = queues.get(message.guild.id);
...
const addSongToQueue = (queue, song) => {
...
const onNewMessageReaction = async (reaction, user) => {
  if (user.bot) {
    return;
  }
  ...
  if (
    reaction.message === optionMessage.message &&
    user.id === optionMessage.userId
  ) {
    let song = null;
    try {
