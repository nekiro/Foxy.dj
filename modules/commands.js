...
const commands = [
    {
        text: 'ping',
        callback: (message) => message.channel.send(`🏓 Reponse latency: ${Date.now() - message.createdTimestamp}ms. API latency: ${client.ws.ping}ms`)
    },
    {
        text: 'play',
...
function showHelp(message) {
    message.reply(commands.map(c => `**${c.text}**${c.description ? ` - ${c.description}` : ''}`).join('\n'))
}

exports.handleCommand = async function(message) {
